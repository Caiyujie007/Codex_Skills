...
| Direct XML Exception    | Tiny one-off or raw mxGraph handoff         | `.drawio` XML                    | Use only when exact XML control is the real requirement. Missing YAML/CLI tooling should trigger Tool Preflight, not an automatic route switch. |
...
Before starting a draw.io task, check that the tools required by the selected route are installed on the operator's machine. Do this up front instead of discovering missing tools after producing partial artifacts.

- Offline YAML/CLI authoring requires `node` 20+ and the skill's script dependencies.
- Desktop-enhanced exports require draw.io Desktop/CLI, usually available as `drawio`.
- Validation and visual checks may require the local validators/viewers used by the chosen route, such as `xmllint` for XML/SVG validation or the platform image/PDF inspection tools.
- Live refinement requires the configured browser/MCP provider.

If a required tool for the requested route is missing, stop and report the missing tool plus the installation action the operator should take. Do not silently replace the requested route or artifact with another method, such as generating a screenshot instead of a Desktop PNG/PDF export, hand-writing a different SVG instead of using the expected renderer, or skipping validation. Continue only after the operator installs the tool, explicitly asks Codex to install it, or explicitly accepts a named fallback.
...
| `direct-xml`       | Tiny XML-only handoff, raw mxGraph edits, or diagrams that genuinely need exact XML control                    | `references/official/xml-reference.md`, `references/official/style-reference.md`, `references/docs/xml-format.md`, `references/upstream/pure-drawio-skill.md`                    |
...
5. Treat live backends as optional refinement providers. If live refinement was not requested, edit the offline YAML bundle. If live refinement was requested but `start_session`, `read_diagram_xml`, or patch capabilities are unavailable, stop and report the missing live provider instead of switching routes silently.
...
If Desktop is unavailable for a requested Desktop-enhanced export, stop and report the missing Desktop/CLI dependency plus the installation action. Deliver a `.drawio`/SVG-only bundle only when the operator explicitly accepts that fallback. For browser handoff, generate a diagrams.net URL from the `.drawio` file:
...
If validation fails, fix the YAML or imported XML first and rerun validation. If an optional export cannot run because Desktop or a live backend is unavailable, report the missing provider and the installation action; provide an offline bundle fallback only when the operator explicitly accepts it.
